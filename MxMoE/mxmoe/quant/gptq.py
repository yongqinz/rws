...
def llama_sequential(args, model, dataloader, dev):
    ...
    pos_emb = cache.get('position_embeddings', None)
    ...
    for i in tqdm(range(len(layers)), desc="Layers"):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                fwd_kwargs = dict(attention_mask=attention_mask, position_ids=position_ids)
...
if __name__ == '__main__':
    ...
    if model_id in ["qwen2_moe_57b", "mixtral"]:
        ...
        # GPTQ quantization is layer-by-layer, force single GPU
        model = load_hf_model(model_id, attn_impl="flash_attention_2")
        # Move model to CPU first if it was loaded across GPUs
        model = model.cpu()
