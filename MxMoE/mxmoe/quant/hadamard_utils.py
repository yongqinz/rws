...
def get_hadK(n, transpose=False) -> tuple[torch.Tensor, int]:
    hadK, K = None, None
    if n % 176 == 0:  # qwen2_moe up
        assert (is_pow2(n // 176))
        K = 176
        hadK = get_had176().T if transpose else get_had176()
...
def apply_exact_had_to_linear(module: torch.nn.Linear, had_dim=-1, output=False):
...
AVA_HAD_LIST = [
    12, 16, 20, 24, 28, 32, 46, 40, 44, 48, 52, 56, 60, 64, 68,
